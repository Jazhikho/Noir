...
public class Chapter : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        CutsceneController[] cutsceneControllers = FindObjectsByType<CutsceneController>(FindObjectsSortMode.None);
        if (cutsceneControllers.Length > 1)
        {
            Debug.LogError("Should be only one CutsceneController");
        }
        else if (cutsceneControllers.Length == 1)
        {
            cutscenes = cutsceneControllers[0];
        }
    }
